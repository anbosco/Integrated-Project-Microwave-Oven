...
// Association of a geometry and its corresponding material.
...
*******************************************************************************/
	if(solve_thermo){
		...
		main_th(Power_tot_rotated_back, Temperature,BC, T_Dir,  T_0,dx_th,h_air,Lx_th,Ly_th,Lz_th,dt_th,step_max_th,
			...
			step_cut_th,nb_probe_th,Pos_probe_th, id,k_heat_x,k_heat_y,k_heat_z,rho,cp,constant,
			geometry_th,step_pos,thermo_domain);
		if(myrank==0){
			for(l=1;l<nbproc;l++){
		...
		for(i=0;i<n_cylinder;i++){
			...
			int Config = 1;
			place_geometry_th(X_th,Y_th, Z_th, prop_temp, Config, geometry_th , dx_th, prop_temp[5],
				k_heat_x,k_heat_y,k_heat_z,rho,cp,x_min_th,y_min_th,z_min_th,Temperature, cyl[i]);
		}
		// Cube
		for(i=0;i<n_cube;i++){
			int j=0;
