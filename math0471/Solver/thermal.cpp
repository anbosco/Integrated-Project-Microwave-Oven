...
void main_th(std::vector<double> &Source_elec, std::vector<double> &Temperature,
  ...
  int step_pos,int thermo_domain){
    ...
    if (get_my_rank() == 0)
        host_work(id, Lx, Ly, Lz, dx, dt, step_max,theta, nb_source, prop_source,BC,T_Dir,T_0,SR,Cut,
          Pos_cut,step_cut,nb_probe,Pos_probe,n_sphere,prop_sphere,n_cylinder,prop_cylinder,n_cube,
          prop_cube,  T_food_init,h_air, x_min_th, y_min_th, z_min_th,dx_electro, X_electro, Y_electro,
          Z_electro,Source_elec,k_heat_x,k_heat_y,k_heat_z,rho,cp,
